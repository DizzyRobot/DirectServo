...
bool LimitCalibrate(bool UseFlash,
		...
		int32_t NewTorqueSearchMagnitude,
		int32_t NewAngleOffsetBuffer)
{
	...
	// since they effectively requested to remove the limiter, and we much also then mark it as calibrated
	...
	FailsafeCalibrated = (((lc.Failsafe.MaxAngle+NewAngleOffsetBuffer) - (lc.Failsafe.MinAngle - NewAngleOffsetBuffer)) >= (0xFFFF/360)*10);
