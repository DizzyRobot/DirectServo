...
bool processFailsafe() {
	...
	int32_t NewAngleOffsetBuffer =  (((uint32_t) torqueOffsetMSByte) << 8) | ((uint32_t) torqueOffsetLSByte);

	FailsafeCalibrated = LimitCalibrate(false,NewTorqueLimit,NewTorqueSearchMagnitude,NewAngleOffsetBuffer);
...
void processUsartCommand() {
	...
	bool MessageForThisController = readByte(&b1) && (b1 == config->controllerId || b1 == broadcastId);
